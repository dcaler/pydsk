"""Run Python in deterministic mode for the M3 gate (pairs with C++ out_Bd/).

Same configuration as ``run_deterministic_M1.py`` (N1=100, N2=400, LS0=500000,
matching the C++ ``dsk_B_det`` binary).  The macro frame now also carries the
M3 energy fields added in Task 3.10 to ``Nation.save_outputs``
(share_energy_green, electricity_price, total_energy_demand,
emissions_total{,_s1,_s2,_energy}, d1_fossil_fuel_demand,
mean_electrification_s1, total_green_capacity, total_brown_capacity).

    py_det_M3.parquet   per-step macro aggregates (bit-identical across runs)
"""
...
def main():
    ...
    print(f"Running Python deterministic (M3), T={args.t_max} ...")
    t0 = time.time()
    out = run_deterministic(args.t_max, dump_firms_every=0)
    print(f"Done in {time.time() - t0:.1f}s.")

    macro_path = args.out_dir / "py_det_M3.parquet"
